  ...
  return (
    <section className=" relative min-h-screen md:h-auto lg:h-full bg-black overflow-hidden justify-between lg:px-15 px-5
      ...
      {/* Lado Izquierdo - Título e info */}
        ...
        <div className='relative flex h-full lg:h-full lg:max-w-360 md:max-w-250 max-w-150 mx-auto flex-col lg:flex-row
                          '>
          {/* Lado Derecho - Formulario */}
          <div className='w-full  h-full bg-[var(--color-navbar)] mt-10 p-10 xl:px-30 xl:pt-18 pb-18 shadow-2xl lg:flex-1 lg:min-w-[520px] 
                          '>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col pt-5 text-lg'>
              ...
              <div className='pb-10'>
                ...
                <textarea 
                  placeholder='Mensaje'
                  {...register('message')}
                  className={`focus:outline-none focus:ring-0 pb-20 resize-none bg-transparent ${inputClass('message')} w-full`}
                  rows={5}
                  maxLength={300}
                  >
                </textarea>
